...
//
//  Created by vicente rodriguez on 31/01/21.
//
...
class BoxPrediction: ObservableObject {
    var id = UUID()
    var rect: CGRect
    var score: Float
    ...
    func getFinalBoxes(rawXArray: [Float], rawYArray: [Float], rawWidthArray: [Float], rawHeightArray: [Float], classPredictions: [Float], imageWidth: Float, imageHeight: Float, cameraSize: CGRect) -> [BoxPrediction] {
        ...
        while remaining.count > 0 {
            ...
            if overlapping.count > 0 {
                ...
                // iPhone
                // height is ok, since we take all the height
                // 474.75000000000006 844.0
                // -42.37500000000005 0.0
                // the width takes 42 pixels at EACH side
                // 474 - 42 - 42 == 390        
                ...
                var xCoord = abs(detectionX - 1) * Float(cameraSize.width)
                xCoord = xCoord - Float(abs(cameraSize.origin.x))
